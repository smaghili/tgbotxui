...
async def _notification_actor_context(
    *,
    ...
    children = 0
    if ctx.is_delegated_admin:
        children = await services.db.count_active_delegated_children(telegram_user_id)
    visible = visible_notification_kinds(
        is_root_admin=ctx.is_root_admin,
        is_delegated_admin=ctx.is_delegated_admin,
        has_child_delegates=children > 0,
    )
    return ctx.is_root_admin, ctx.is_delegated_admin, children > 0, visible
...
async def open_bot_notification_settings(
    ...
    _, _, _, visible = await _notification_actor_context(
