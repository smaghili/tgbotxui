...
async def start_bind(message: Message, state: FSMContext, settings: Settings, services: ServiceContainer) -> None:
    ...
    if not panels:
        await answer_with_admin_menu(message, t("bind_no_panel", lang), settings=settings, services=services, lang=lang)
        return
...
async def bind_finalize(message: Message, state: FSMContext, services: ServiceContainer) -> None:
    ...
    try:
        ...
        await answer_with_admin_menu(message, f"{t('bind_failed', lang)}:\n{exc}", settings=settings, services=services, lang=lang)
        return
...
async def sync_all(message: Message, settings: Settings, services: ServiceContainer) -> None:
    ...
    await answer_with_admin_menu(message, t("sync_done", lang), settings=settings, services=services, lang=lang)
...
async def bind_by_command(message: Message, settings: Settings, services: ServiceContainer) -> None:
    ...
    try:
        ...
        await answer_with_admin_menu(message, f"Error in bind:\n{exc}", settings=settings, services=services, lang=lang)
        return
    await answer_with_admin_menu(
        message,
