...
class FinanceRepository:
    ...
    async def get_wallet(self, telegram_user_id: int) -> dict[str, Any] | None:
        assert self.db.conn is not None
    ...
    async def get_pricing(self, telegram_user_id: int) -> dict[str, Any] | None:
        assert self.db.conn is not None
        cur = await self.db.conn.execute(
            """
            SELECT
    ...
    async def get_transaction(self, transaction_id: int) -> dict[str, Any] | None:
        assert self.db.conn is not None
        cur = await self.db.conn.execute(
            """
    ...
    async def has_refund(self, transaction_id: int) -> bool:
        assert self.db.conn is not None
        cur = await self.db.conn.execute(
            """
            SELECT id
    ...
    async def get_sales_report(self, telegram_user_id: int) -> dict[str, int]:
        assert self.db.conn is not None
        cur = await self.db.conn.execute(
            """
            SELECT
    ...
    async def get_scope_sales_totals(self, telegram_user_ids: list[int]) -> dict[str, int]:
        ...
        if not telegram_user_ids:
            return {"total_sales": 0, "total_refunds": 0, "net_sales": 0, "total_transactions": 0}
    ...
    async def get_overall_report(self) -> tuple[dict[str, Any], dict[str, Any], dict[str, Any]]:
        assert self.db.conn is not None
        cur = await self.db.conn.execute(
            """
            SELECT
