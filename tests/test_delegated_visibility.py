...
class DelegatedVisibilityTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_delegated_create_client_charges_wallet_and_sets_numeric_comment(self) -> None:
        class Settings:
            admin_ids = set()
        ...
        service = AdminProvisioningService(
            ...
            usage_service=FakeUsageService(),  # type: ignore[arg-type]
        )
        ...
        self.assertEqual(panel_service.comment, "55")
        self.assertEqual(len(financial_service.charge_calls), 1)
        self.assertEqual(result["wallet_charge_amount"], 1000)
